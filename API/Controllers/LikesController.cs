...
namespace API.Controllers {
    ...
    public class LikesController : BaseApiController {
        ...
        public async Task<ActionResult> AddLike (string username) {
            ...
            var likedUser = await userRepository.GetUserByUsernameAsnyc (username);
            var sourceUser = await likesRespository.GetUserWithLike (sourceUserId);

            if (likedUser == null) return NotFound ();
            if (sourceUser.UserName == username) return BadRequest ("You cannot like yourself");

            var userLike = await likesRespository.GetUserLike (sourceUserId, likedUser.Id);
