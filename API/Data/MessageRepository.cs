...
namespace API.Data
{
    public class MessageRepository : IMessageRepository
    {
        ...
        public async Task<Message> GetMessage(int id) => await _context.Messages.Include(u=> u.Sender).Include(u=>u.Recipient).SingleOrDefaultAsync(x =>x.Id ==id);
        ...
        public async Task<IEnumerable<MessageDto>> GetMessagesThread(string currentUsername, string recipientUsername)
        {
            ...
            && m.Sender.UserName
             == currentUsername && m.SenderDeleted == false).OrderBy(m => m.MessageSent).ToListAsync();
